...
class ShowFrame():
	...
	def showContour(self):
			...
			if self.AreaOfContourOuter[iterationOuter] >5000:
				...
				for iterationInner in range(0,len(self.IndexOfZeroParentInner)):
					if self.AreaOfContourInner > 1700:
						self.cnt_OringInner  = contours[self.IndexOfZeroParentInner[iterationInner]]
		...
		self.radiusOuter=[]
		self.radiusInner=[]
		for iterationOuter in range(0,len(self.outRadIndexForLogic)):
			self.radiusOuter.append(self.oringLogic(self.outRadIndexForLogic[iterationOuter]))
			
		#~ print "length of radius",len(self.radiusOuter)
		#~ for iterationInner in range(0,len(self.inRadIndexForLogic)):
			#~ self.radiusInner.append(self.oringLogic(self.inRadIndexForLogic[iterationInner]))
		#~ print "inner radius",self.radiusInner
		#~ print "Outer radius",self.radiusOuter
		#~ print "len",len(self.radiusOuter)
		
	def oringLogic(self,i):
		#~ print "iteration outer",iterationOuter
		for i in range(0,len(self.cnt_OringOuter)):
			self.xOuter = self.cnt_OringOuter[i]
		
			self.xOuter1 = self.xOuter[:,:,0]
			#~ print "The outer",self.xOuter
			self.x_OringOuter = self.xOuter1[:,0]
			#~ print "The  oring outer is",self.x_OringOuter
